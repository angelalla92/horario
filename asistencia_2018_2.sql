...
-- create procedure listarPracticantes()
-- BEGIN
-- select dni,apePAterno, apeMaterno,nombres,fecNacimiento,sexo,codTurno_fk,descripcion from practicantes;
--END

CREATE DEFINER=`root`@`localhost` PROCEDURE `listarPracticantes`()
BEGIN
select dni,apePAterno, apeMaterno,nombres,fecNacimiento,sexo,codTurno_fk,descripcion from practicantes;
END
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `registarpracticantes`(dni varchar(10), apePaterno varchar(50), apeMaterno varchar(50), nombres varchar(50), fechaNacimiento varchar(12),sexo varchar(2),codTurno_fk varchar(2), descripcion varchar(50))
begin
insert into practicantes values(dni,apePaterno,apeMaterno,nombres,fechaNacimiento,sexo,codTurno_fk,descripcion);
end